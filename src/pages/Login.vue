<template>
  <q-page class="flex bg-image flex-center">
    <q-card flat :style="$q.screen.lt.sm ? { width: '80%' } : { width: '30%' }">
      ...
      <q-card-section>
        <q-form class="q-gutter-md" @submit.prevent="login">
          <q-input v-model="model.identifier" autofocus lazy-rules outlined
      ...
      (val) =>
        ...
        $gettextInterpolate(
          $gettext('Please use minimum %{n} characters'),
          {
            ...
            <template #append>
              <q-icon :name="showcredential ? 'mdi-eye' : 'mdi-eye-off'" @click="showcredential = !showcredential" />
            </template>
          </q-input>

          <div>
            <q-btn class="full-width" type="submit" color="primary" :loading="loading" :disabled="!isValid">
        ...
        <q-toolbar class="q-pa-md">
          <q-btn-dropdown ref="changeLanguage" flat stretch>
            ...
            <q-list>
              <q-item v-for="(language, key) in $language.available" :key="key" :active="key === $language.current"
                clickable @click="changeAppLanguage(key)">
                {{ language }}
              </q-item>
            </q-list>
          </q-btn-dropdown>
    ...
    <q-card flat :color="$q.dark.isActive ? 'blue-grey-9' : 'green'"
      :style="$q.screen.lt.sm ? { width: '80%' } : { width: '50%' }">
      <q-card-section>
        <div class="wrap" color="green" style="display: flex; flex-direction: column">
          <div class="top">
            <div class="head">
              <img class="logo" src="../assets/brand.svg" alt="" />
            </div>
          ...
          <div class="-inner-content mobile-hide" style="margin-left: 10%">
            <div style="margin-top: 10px; font-weight: bolder; font-size: xxx-large;">
              <translate>app.passport.intro-1</translate><br />
              <translate>app.passport.intro-2</translate>
            </div>
            <q-separator></q-separator>
            <div style="margin-top: 15px; font-weight: normal; font-size: medium; ">
...
export default {
  ...
  setup() {
    ...
    const model = reactive({ identifier: '', credential: '', type: 0 })
    const changeLanguage = ref(null)

    const isValid = computed(() => {
      return model.identifier.trim() !== '' && model.credential.length > 3
    })
